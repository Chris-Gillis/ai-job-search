// Data source: SmartRecruiters public Posting API. No authentication required.
//
// SmartRecruiters is a multi-tenant ATS: every company has its own board keyed by
// a company identifier (case-sensitive, e.g. "Visa"). There is no global
// cross-company search, but unlike some boards SmartRecruiters supports
// server-side keyword search (q=) and offset/limit pagination, so the search
// command pages through the postings and can push the keyword filter server-side.

export const BASE_URL = "https://api.smartrecruiters.com/v1/companies"
