#!/usr/bin/env bun
// Self-contained CLI for searching Greenhouse public job boards, for any company
...
//
// Greenhouse is board-scoped: every request targets one company's board via its
// board token (usually the company name lowercased, e.g. "stripe"). There is no
...
const HELP = `greenhouse-cli — search Greenhouse public job boards (any company, any country)
...
SEARCH FLAGS
  --board, -b <token>     Board token(s). REQUIRED. Comma-separate to search several
                          companies, e.g. "stripe,figma". The token is usually the
                          company name lowercased; find it in the careers-page URL
                          (job-boards.greenhouse.io/<token> or boards.greenhouse.io/<token>).
  --query, -q <text>      Keywords (matched in title, department, and description).
...
EXAMPLES
  bun run src/cli.ts search -b stripe -q engineer --format table
  bun run src/cli.ts search -b stripe,figma -q "software engineer" -l remote --limit 10
  bun run src/cli.ts detail 6317879 --board stripe --format plain

Job Board data is public — no authentication required.
`
  ...
  if (cmd === "search") {
    ...
    if (!board) {
      process.stderr.write(
        JSON.stringify({
          error: 'the --board/-b flag is required (a Greenhouse board token, e.g. -b stripe; comma-separate for several)',
          code: "NO_BOARD",
        }) + "\n",
      )
      return 1
